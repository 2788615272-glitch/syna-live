...
| 自定义形象 | 可自由增删、重命名和替换的动态表情立绘、说话立绘、缩放和 OBS 透明舞台 |
| AI 对话 | 流式回复、首段提前朗读、火山方舟及其他 OpenAI 兼容模型 |
| 桌面陪伴 | 透明悬浮立绘、同步对话历史、打字/按下说话/自动聆听 |
| 桌面视觉 | 单脑直接看图 / 双脑持续观察，支持画面突变反应和空闲主动开口 |
...
3. 在“角色人设”中添加、删除、重命名或替换任意数量的表情立绘，模型会按当前表情名自动选择；
4. 点击“弹出桌面陪伴立绘”，选择打字、按下说话或自动聆听；
5. 可在“语音”中继续使用免费本机模式，或配置独立 ASR/TTS 模型；
6. 可选在“视觉”中选择单脑直接看图，或双脑持续观察；画面只发送给你配置的模型接口；
7. 在“直播”中复制舞台地址，添加为 OBS 浏览器源；
8. 可选填写 B 站房间号并开启弹幕自动回复。

回复会边生成边显示，第一段可朗读内容会提前送入 TTS。用户开始说话时，当前朗读与同一回复剩余语音会立即停止。
API/火山 ASR 在检测到一句话结束后的约 0.7 秒静音时自动收口；桌面窗口与主控制台共享同一份本地对话历史。
双脑视觉不仅会响应切换窗口等明显变化，也会在视频等稳定场景中按可调间隔自然聊一句；主窗口位于后台时观察计时仍会继续。
